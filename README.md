...
ZCode 桌面端远程控制的手机伴侣 App。电脑端出示二维码，手机扫码一次导入，即可长期使用；多台设备并行管理，一个界面随时切换，还能在手机上管理自动化任务、领取活动权益、查看应用用量。
...
- **功能工具箱** —— 会话顶栏一键展开：自动化任务、活动权益、用量统计收拢一处，待办提醒以红点聚合呈现
- **自动化任务管理** —— 闲时任务与定时任务在手机上创建、编辑、启停与删除；执行状态与排队情况实时上卡，失败原因一目了然；计划时间以「每周五 18:00」这类自然语言显示
- **活动权益领取** —— 限时活动套餐在手机上一键领取，领取成功即见票券弹窗
- **应用用量统计** —— 汇总读数、30 天使用热力图、每日模型趋势与模型占比；点按任意一天查看当日明细，支持全部 / 近 7 天 / 近 30 天切换
...
- **一键刷新与自动恢复** —— 会话异常时手动重载，连续失败交给自动重载兜底；长时间挂在后台后回到前台也会自动重连
- **生物识别门禁** —— 指纹 / 面容锁定应用，开启与关闭均需验证；锁屏采用品牌主视觉
- **全新界面设计** —— 工业风视觉全面焕新：锁屏、扫码、导入、会话与设置逐屏精工重制
- **深浅色主题** —— 跟随系统或手动选择深色 / 浅色，控制会话内的网页一并换肤
- **中英双语** —— 应用内一键切换 中文 / English，或跟随系统语言
- **设置页** —— 外观（语言、主题）、安全与保活、通知偏好集中管理，设备列表回归纯操作
...
4. 顶栏工具箱进入自动化任务、活动权益与用量统计
5. 任务等待批准或完成时，系统通知会即时送达；相关偏好可在设置页调整
